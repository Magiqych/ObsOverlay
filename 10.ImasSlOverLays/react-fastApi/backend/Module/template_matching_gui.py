    ...
    if not ret:
        ...
        if template_matching(cv2_image, get_template_by_name("ComfirmButton.png"),0.8):
            ...
            if template_matching(cv2_image, get_template_by_name("SelectGuest.png"),0.8)\
                ...
                message = {"SongName": selectedItem.song_name, "Level": selectedItem.level}
                requests.post(url, json=message)
            #リボンとOKボタンが表示されたらスコアOCRを行う
            if(template_matching(cv2_image, get_template_by_name("Score_UI4.png"),0.9)\
                and template_matching(cv2_image, get_template_by_name("OkButton.png"),0.9)):
                hoge = 0
